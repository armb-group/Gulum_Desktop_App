...
export default function StudentLectureAudit() {
  ...
  const subjects = apiSubjects ?? STATIC_SUBJECTS;
    ...
    <RoleShell role="student" title="Lecture Progress" subtitle="Your syllabus coverage">
        ...
        {subjects.map((s: any, i: number) => {
          ...
          return (
            <div key={i} className="flex-1 rounded-2xl border border-border bg-surface p-3 flex flex-col items-center">
              ...
              <span className="text-muted-foreground text-xs mt-1 text-center">{s.name}</span>
            </div>
          );
        })}
      </div>
      ...
      <div className="space-y-4">
        {subjects.map((subject: any, si: number) => {
          ...
          const pct = Math.round((subject.totalCompleted / subject.totalHours) * 100);
            ...
            <Card key={si} className="p-5 bg-surface border-border">
              ...
              {isOpen && (
                <div className="mt-4 space-y-3">
                  {subject.modules.map((mod: any, mi: number) => {
                    const modPct = Math.round((mod.completed / mod.total) * 100);
                    ...
                    return (
                      <div key={mi} className="rounded-xl border border-border bg-background p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <span className="font-semibold text-foreground flex-1">{mod.title}</span>
                        </div>
                        <p className="text-primary font-semibold text-sm mb-2">{mod.completed}/{mod.total} hrs completed</p>
